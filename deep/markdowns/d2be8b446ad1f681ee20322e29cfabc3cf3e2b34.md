...
> 视频管线: Local (Node.js ≥ 22)（适用: HTML/CSS/GSAP programmatic video — kinetic typography, product promos, launch reels, custom motion graphics, registry blocks (data charts, grain overlays, shader transitions), website-to-video workflows, and rigged SVG character animation. Consumed via `npx hyperframes`; no monorepo checkout needed.）

## 基本信息

| 字段 | 内容 |
|---|---|
| 名称 | openmontage/hyperframes |
