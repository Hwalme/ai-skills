...
| 字段 | 内容 |
|---|---|
| 名称 | k-kolomeitsev/data-structure-protocol |
| 链接 | https://github.com/k-kolomeitsev/data-structure-protocol |
...
> [!WARNING]
> **Deprecated.** This repository is no longer developed. The current skill is
> **[dsp-codegen](https://github.com/Loqira-Labs/dsp-codegen)** — spec-driven polyglot code
> generation from the DSP graph (the graph works as compiler IR, not only as memory).
> Install it with one command into Claude Code / Cursor / Codex / Hermes / OpenClaw.
> The new skill is compatible with this one: existing `.dsp/` graphs and `@dsp` markers keep
> working, though not every feature of the old skill is carried over.
